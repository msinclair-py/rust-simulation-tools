...
class TestUnwrapSystem:
    ...
    def test_float32_support(self):
        """Test that float32 arrays are handled correctly."""
        trajectory = np.array([[
            [1.0, 1.0, 1.0],
            [1.5, 1.0, 1.0],
        ]], dtype=np.float32)
        ...
        assert unwrapped.dtype == np.float32, "Output dtype should match input dtype"
        assert np.allclose(unwrapped[0], trajectory[0]), \
            "Float32 coordinates should be handled correctly"
