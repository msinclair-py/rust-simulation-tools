...
pub const PROTEIN_RESIDUES: &[&str] = &[
    ...
    "NHIS", "NHID", "NHIE", "NHIP", "NILE", "NLEU", "NLYS", "NMET", "NPHE",
    "NPRO", "NSER", "NTHR", "NTRP", "NTYR", "NVAL",
    // C-terminal variants
    "CALA", "CARG", "CASN", "CASP", "CCYS", "CCYX", "CGLN", "CGLU", "CGLY",
    "CHIS", "CHID", "CHIE", "CHIP", "CILE", "CLEU", "CLYS", "CMET", "CPHE",
...
pub fn is_protein_residue(name: &str) -> bool {
    ...
    PROTEIN_RESIDUES.iter().any(|&r| r.eq_ignore_ascii_case(trimmed))
}

/// Check if a residue name is a water residue.
pub fn is_water_residue(name: &str) -> bool {
    let trimmed = name.trim();
    WATER_RESIDUES.iter().any(|&r| r.eq_ignore_ascii_case(trimmed))
}

/// Check if an atom name is a backbone atom.
pub fn is_backbone_atom(name: &str) -> bool {
    let trimmed = name.trim();
    BACKBONE_ATOMS.iter().any(|&a| a.eq_ignore_ascii_case(trimmed))
}
